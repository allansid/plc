...
//    }
    ...
    private void drawHorizontalWall(Graphics g, int x, int y, int size, int wall_length){
        int index = 1;
        while(index < wall_length){
            g.drawImage(wall_center_img, x+index*size, y, this);
            index++;
            g.drawImage(wall_right_img, x+(index*size), y, this);
            index++;
        }
    }
